    ...
    chrome.storage.sync.get(["firstColor"], function(result){
        ...
        chrome.storage.sync.get(["secondColor"], function(result){
            ...
            chrome.storage.sync.get(["thirdColor"], function(result){
                ...
                var selectorBar = "body > div.view-scaffold.ng-scope > div.ui.borderless.top.fixed.inverted.hidden.menu.blue.fix-height.ng-scope";
                var selectorSideBarHeader = "body > div.view-scaffold.ng-scope > div.ui.pushable.right.raised.sidebar.segments.short-action.overlay.visible.right-sidebar-container.right.toggle-right-sidebar-hide > div.ui.segment.right-sidebar-up > div > div.ui.top.right.pointing.content.dropdown.adjust-responsive-padding.speed-dropdown > h4";
                var selectorSideBarSec = "body > div.view-scaffold.ng-scope > div.ui.pushable.right.raised.sidebar.segments.short-action.overlay.visible.right-sidebar-container.right.toggle-right-sidebar-hide > div.ui.segment.right-sidebar-down > h4";
                var firstSideBarIcon = "#speedDialSettingsIcon";
                var secondSideBarIcon = "body > div.view-scaffold.ng-scope > div.ui.pushable.right.raised.sidebar.segments.short-action.overlay.visible.right-sidebar-container.right.toggle-right-sidebar-hide > div.ui.segment.right-sidebar-down > i";
                var chatSideBarSel = "#headerIdChat";
                var chatSideBarIcon = "#headerIdChat > i";

                // check elements exists
                var checkExist = setInterval(function() {
                    if ($(selectorBar).length && $(selectorSideBarHeader).length && $(selectorSideBarSec).length && $(chatSideBarSel).length) {
                        // set div style
