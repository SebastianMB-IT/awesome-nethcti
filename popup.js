...
$( document ).ready(function() {
    ...
    for (let i = 0; i < colors.length; i++) {
        if (n == 0) {
            let btnColors = colors[i].colors.join(',');
            $("#gradientsContainer").append('<div id="row-' + i + '" class="color-blocks-container"><div>');
            $("#row-" + i).append('<button id="first-' + i + '" class="btn btn-primary defined-colors color-left" style="background: linear-gradient(to right, ' + btnColors + ');"></button>');
            n++;
        } else {
            let btnColors = colors[i].colors.join(',');
            $("#row-" + (i-1)).append('<button id="second-' + i + '" class="btn btn-primary defined-colors color-left" style="background: linear-gradient(to right, ' + btnColors + ');"></button>');
            n = 0;
        }
    }

    chrome.storage.sync.get(["colors"], function(result){
        result.colors[0] ? $("#firstColor").val(result.colors[0]) : $("#firstColor").val('') ;
        result.colors[1] ? $("#secondColor").val(result.colors[1]) : $("#secondColor").val('') ;
        result.colors[2] ? $("#thirdColor").val(result.colors[2]) : $("#thirdColor").val('') ;
    });
    ...
    $(".defined-colors").click(function() {
        let colors = $( this ).attr("style").match(/#\b[0-9A-F]{6}\b/gi);
        let colorsObj = {
            colors: {}
        };
        for (let i = 0; i < colors.length; i++) {
    ...
    $("#buttonSave").click( function () {
        ...
        let first = $("#firstColor").val();
        if (first != "") {
            colors.colors[0] = first;
        }
        let second = $("#secondColor").val();
        if (second != "") {
            colors.colors[1] = second;
        }
        let third = $("#thirdColor").val();
        if (third != "") {
            colors.colors[2] = third;
        }
